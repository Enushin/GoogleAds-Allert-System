...
| 優先度 | アクション | 目的 | オーナー | 完了条件 |
| --- | --- | --- | --- | --- |
| Blocker | ログレベル INFO 以上をSlack通知処理とGoogle Ads API呼び出しの成功/失敗で必ず出力し、Cloud Loggingなどで手動監視できる状態を維持する。 | APIと通知の成功可否を即座に把握し、障害時に復旧判断を誤らないため。 | SRE担当 | Slack/Cloud Logging上で成功・失敗ログが確認できる。 |
| Blocker | `python -m google_ads_alert run --dry-run` の結果を日次で保存し、予測値と実績費用を比較する軽量なレビューメモを運用チーム内で共有する。 | 予測ロジックが実運用値と乖離していないか、最低限の品質ガードをかける。 | 運用担当 | 日次メモのテンプレートが完成し、初日分が保存されている。 |
| Blocker | 重大障害（通知送信不可、API失敗連続）を検知した場合は、[障害対応ランブック](incident_response_runbook.md) に従い手動エスカレーションする。 | 自動化がない状態でもユーザー影響を最小化する。 | 当番エンジニア | ランブック該当節の確認済みサインオフが残っている。 |

### ローンチ即応フロー（報告タスクを含めない）

1. **Slackログを唯一の証跡とみなす** — APIレスポンスや予測値の詳細レポートは収集しない。Slackログが残っていれば監視要件クリアと判断する。
2. **失敗検知はランブック優先** — 自動集計やダッシュボードは導入しない。手動で `grep "ERROR"` を実行し、異常時はランブック通りに連絡する。
3. **レビューは軽量メモだけ** — 日次レビューメモはGoogleドキュメント/スプレッドシートなど既存ツールにチェックボックスを設け、数値の整形やグラフ化は行わない。
4. **Blocker解消後は即Go判定へ進む** — 追加のレポーティング作業やグラフ作成を理由にローンチを延期しない。

> **ローンチ判定条件**: 上記Blockerが揃えば監視面のGo条件は満たす。ダッシュボードや自動集計が未整備でもローンチを止めない。
